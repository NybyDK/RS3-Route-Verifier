...
const XPThresholdsInvention = [
    ...
    19656, 23546, 28138, 33520, 39809, 47109, 55535, 64802, 77190, 90811,
    106221, 123573, 143025, 164742, 188893, 215651, 245196, 277713, 316311,
    358547, 404364, 454796, 509259, 568254, 632019, 700797, 774834, 854383,
    946227, 1044569, 1149696, 1261903, 1381488, 1508756, 1644015, 1787581,
    1939773, 2100917, 2283490, 2476369, 2679907, 2894505, 3120508, 3358307,
    ...
    52247435, 54303504, 56412678, 58575823, 60793812, 63067521, 65397835,
    ...
    107764216, 111125230, 114558777, 118065845, 121647430, 125304532, 129038159,
    132849323, 136739041, 140708338, 144758242, 148889790, 153104021, 157401983,
    161784728, 166253312, 170808801, 175452262, 180184770, 185007406, 189921255,
    194927409,
];
...
module.exports = class Player {
    ...
    gainXP(skill, xp) {
        ...
        if (this.skills[skill] === undefined) {
            throw new Error(
                `Skill '${skill}' does not exist during step ${this.step}`,
            );
        }

        if (xp === undefined || xp < 0) {
            throw new Error(
                `Invalid XP amount '${xp}' for skill '${skill}' during step ${this.step}`,
            );
        }
    ...
    removeItem(itemName, quantity = 1) {
        ...
        if (!item) {
            throw new Error(
                `Item '${item}' not found during step ${this.step}`,
            );
        }
    ...
    completeQuest(questName, xpDistribution = {}) {
        ...
        if (this.quests.includes(questName)) {
            throw new Error(
                `Quest '${questName}' is already completed during step ${this.step}`,
            );
        }
        ...
        for (const [skill, level] of Object.entries(
            ...
            if (
                this.calculateLevel(this.skills[skill], skill === "invention") <
                level
            ) {
                throw new Error(
        ...
        if (
            rewardXP.length !== distributedXP.length ||
            !rewardXP.every((xp, index) => xp === distributedXP[index])
        ) {
            throw new Error(
        ...
        for (const [skill, XPValue] of Object.entries(xpDistribution)) {
            ...
            for (const XP of XPArray) {
                ...
                if (reward.skills && !reward.skills.includes(skill)) {
                    throw new Error(
                        `XP for ${skill} is not allowed for ${questName} during step ${this.step}`,
                    );
                }
